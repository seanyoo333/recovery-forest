    ...
    IF new.raw_app_meta_data IS NOT NULL AND new.raw_app_meta_data ? 'provider' THEN
        ...
        IF new.raw_app_meta_data ->> 'provider' = 'email' OR new.raw_app_meta_data ->> 'provider' = 'phone' THEN
            ...
            IF new.raw_user_meta_data ? 'name' THEN
                ...
                VALUES (new.id, new.raw_user_meta_data ->> 'name', 'user_' || substr(md5(new.id::text), 1, 8), 'patient', (new.raw_user_meta_data ->> 'marketing_consent')::boolean);
            ELSE
                -- Otherwise, set a default name and opt-in to marketing
                INSERT INTO public.profiles (profile_id, name, username, role, marketing_consent)
                VALUES (new.id, 'Anonymous', 'user_' || substr(md5(new.id::text), 1, 8), 'patient', TRUE);
            END IF;
        ELSE
            ...
            VALUES (new.id, new.raw_user_meta_data ->> 'full_name', 'user_' || substr(md5(new.id::text), 1, 8), 'patient', new.raw_user_meta_data ->> 'avatar_url', TRUE);
        END IF;
    END IF;
