...
test.describe("Delete Profile", () => {
  ...
  test("should delete account after confirmation", async ({ page }) => {
    ...
    await page.waitForSelector("#confirm-delete", { state: "visible" });
    ...
    await test.step("check both confirmations", async () => {
      // Check the first confirmation checkbox (한국어 텍스트)
      await page.getByLabel(/계정을 삭제하겠습니다/i).check();
      // Check the second confirmation checkbox (한국어 텍스트)
      await page
        .getByLabel(/삭제한 계정은 복구할 수 없음을 이해했습니다/i)
        .check();
    });
    ...
    await test.step("submit deletion and confirm redirect", async () => {
      // Click the delete account button (한국어 텍스트)
