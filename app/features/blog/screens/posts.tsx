...
export default function Posts({
  ...
  // 모든 고유 카테고리 추출
  const categories = [...new Set(frontmatters.map((post) => post.category))];

  // 현재 선택된 카테고리 상태 (실제로는 URL 파라미터나 상태 관리 사용)
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);

  // 선택된 카테고리에 따라 포스트 필터링
  const filteredPosts = selectedCategory
    ? frontmatters.filter((post) => post.category === selectedCategory)
    : frontmatters;
