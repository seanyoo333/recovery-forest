...
export const METRIC_DEFINITIONS = {
  ...
  lmr: { label: "림프구 대비 단핵구 비율", color: "hsl(199, 89%, 48%)", unit: "" },
  nlr: { label: "호중구 대비 림프구 비율", color: "hsl(142, 76%, 36%)", unit: "" },
  ...
  vitamin_d3: { label: "Vitamin D3", color: "hsl(84, 81%, 44%)", unit: "ng/mL" },
  platelet: { label: "혈소판", color: "hsl(224, 76%, 48%)", unit: "10³/µL" },
  ast: { label: "아스파르테이트 아미노전달효소", color: "hsl(15, 100%, 50%)", unit: "U/L" },
  alt: { label: "알라닌 아미노전달달효소", color: "hsl(286, 72%, 45%)", unit: "U/L" },
  egfr: {
    label: "추정 사구체 여과율",
    color: "hsl(46, 97%, 45%)",
    unit: "mL/min/1.73㎡",
...
export const PRIMARY_METRICS: Array<keyof typeof METRIC_DEFINITIONS> = ["glucose", "hgba1c", "crp"];
...
type RawBloodTestResult = Database["public"]["Tables"]["blood_test_results"]["Row"] & {
  blood_test_types:
    | Array<{
...
export async function getBloodTestOverview(client: SupabaseClientType, userId: string): Promise<BloodTestOverview> {
  const { data, error } = await client
  ...
  const parsedResultsWithId =
    data?.flatMap((item: any) => {
      ...
      const metricKey = normalizeStandardName(typeInfo.standard_name) as keyof typeof METRIC_DEFINITIONS;
      const isTumorMarker = metricKey.startsWith("tumor_marker_");
      if (isTumorMarker || metricKey in METRIC_DEFINITIONS) {
        const definition = isTumorMarker ? METRIC_DEFINITIONS[metricKey] : METRIC_DEFINITIONS[metricKey];
        if (!definition) {
          return [];
        }
        const unit = item.result_unit ?? typeInfo.unit ?? definition?.unit ?? null;
        return [
          {
  ...
  const deduplicatedResults = Array.from(uniqueResults.values()).map(({ result_id, ...rest }) => rest);

  const metricsByDate = new Map<string, Record<string, { value: number; unit: string | null }>>();
  ...
  let chartData: BloodTestChartPoint[] = Array.from(metricsByDate.entries())
    .map(([date, values]) => ({
      date,
      ...Object.fromEntries(Object.entries(values).map(([metric, value]) => [metric, value.value])),
    }))
      ...
      if (currentValue === undefined || currentValue === null) {
        ...
        for (let i = currentIndex - 1; i >= 0; i--) {
          ...
          if (prevMetricValue !== undefined && prevMetricValue !== null) {
            prevValue = typeof prevMetricValue === "number" ? prevMetricValue : Number(prevMetricValue);
            break;
          }
        }
        // 뒤쪽에서 가장 가까운 값 찾기
        ...
        for (let i = currentIndex + 1; i < chartData.length; i++) {
          ...
          if (nextMetricValue !== undefined && nextMetricValue !== null) {
            nextValue = typeof nextMetricValue === "number" ? nextMetricValue : Number(nextMetricValue);
            break;
          }
        }
  ...
  tumorMarkers.forEach((metric) => {
    ...
    if (summary) {
      latestSummary.push({
        metric,
        value: summary.value,
        unit: summary.unit ?? METRIC_DEFINITIONS[metric as keyof typeof METRIC_DEFINITIONS]?.unit ?? "",
        label: METRIC_DEFINITIONS[metric as keyof typeof METRIC_DEFINITIONS]?.label ?? metric,
  ...
  const referenceRanges: Record<string, { min: number | null; max: number | null }> = {};
  ...
  availableDatesSet.forEach((dateKey) => {
    ...
    const dateTumorMarkers = deduplicatedResults
      .filter((r) => r.test_date === dateKey && r.metricKey.startsWith("tumor_marker_"))
      .map((r) => r.metricKey)
    ...
    dateTumorMarkers.forEach((metricKey) => {
      const result = deduplicatedResults.find((r) => r.test_date === dateKey && r.metricKey === metricKey);
...
export const getBloodTestTypesByStandardNames = async (
  ...
  const normalizedNames = standardNames.map((name) => normalizeStandardName(name));
